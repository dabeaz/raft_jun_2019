...
def handle_client(ch, shutdown: threading.Event):     # Ch already created (by server)
    server = KVHandler(ch)
    server.run(shutdown)

def run_kv_server(address, shutdown: threading.Event):
    import message
    message.run_channel_server(address, handle_client, shutdown)
...
def smoke_test():
    ...
    srv = threading.Thread(target=run_kv_server, args=(address, shutdown))
    srv.start()
